...
class NotificationService {
  ...
  void _onForegroundTap(NotificationResponse details) {
    // ignore: avoid_print
    _log.fine('_onForegroundTap(${details.id}) >> '
        'action ${details.actionId} tapped with'
        ' payload: ${details.payload}');
    if (details.input?.isNotEmpty ?? false) {
      // ignore: avoid_print
      _log.fine(
        '_onForegroundTap(${details.id}) >> '
        'action ${details.actionId} tapped with input ${details.input}',
      );
    }
    if (details.id != null) {
      _ackAll([details.id!]);
  ...
  void _onBackgroundTap(NotificationResponse details) {
    // ignore: avoid_print
    _log.fine('_onBackgroundTap(${details.id}) >> '
        'action ${details.actionId} tapped with'
        ' payload: ${details.payload}');
    if (details.input?.isNotEmpty ?? false) {
      // ignore: avoid_print
      _log.fine(
        '_onBackgroundTap(${details.id}) >> '
        'action ${details.actionId} tapped with input ${details.input}',
      );
    }
    if (details.id != null) {
