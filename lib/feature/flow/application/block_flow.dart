...
class BlockFlow extends Flow {
    ...
    return guardSync(
      () {
        ...
        for (final name in expr.getUsedVariables()) {
          ...
          expr.setStringVariable(name, match.value.data.toString());
        }
        // TODO: Set variable values from tags before evaluation
  ...
  Stream<FlowEvent> _handle(
    ...
    final isExpired = state.isExpired(trigger.ttl);
    ...
    _model = _model.copyWith(
      state: BlockState.fromTags(
        ...
        value: value,
        repeated: repeated,
        lastChanged: changed || isExpired ? DateTime.now() : state.lastChanged,
      ),
    );

    if (repeated < trigger.debounceUntil) return;

    final shouldRepeat = repeated <= trigger.repeatUntil;

    if (shouldRepeat || isExpired) {
